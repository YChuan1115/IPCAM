...
void send_snapshot(int fd) {
  ...
  int file_fd;
  char *file_name="/tmp/snapshot.jpg";
 ...
 //---added by joseph , 2012, May 21 -----
      /* open file for write */
    if( (file_fd = open(file_name, O_CREAT|O_WRONLY|O_TRUNC, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH)) < 0 ) {
      DBG("could not open the file %s\n",file_name);
     
    }

    /* save picture to file */
    if( write(file_fd, frame, frame_size) < 0 ) {
...
void command(int id, int fd, char *parameter) {
  ...
  for ( i=0; i < LENGTH_OF(out_cmd_mapping); i++ ) {
    ...
    if ( strcmp(out_cmd_mapping[i].string, command) == 0 ) {
      res = output_cmd(id, out_cmd_mapping[i].cmd, ivalue);  // 2012-11-20 IPCAM keypoint , hsian checked
      break;
    }
    /*
    // 2012-11-20  hsian, check ST& command , set tilt any angle command 
    if ( strncmp("ST&" , command,3) == 0 ) {
    
      out_cmd_mapping[i].cmd = OUT_CMD_ST ;
      strcpy( out_cmd_mapping[i].string , command);
   
      res = output_cmd(id, out_cmd_mapping[i].cmd, ivalue);  
      break;
    }
   // 2012-11-20  hsian, check SP& command , set pan any angle command 
    if ( strncmp("SP&" , command,3) == 0 ) {
    
      out_cmd_mapping[i].cmd = OUT_CMD_SP ;
      strcpy( out_cmd_mapping[i].string , command);
