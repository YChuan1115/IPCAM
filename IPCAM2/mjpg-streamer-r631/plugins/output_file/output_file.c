...
static int fd, delay, bytes,only_one;
...
void *worker_thread( void *arg ) {
  ...
  while ( ok >= 0  && !pglobal->stop ) {
    ...
    */
    ...
    DBG("writing file: %s\n", buffer2);
    //joseph modified, 2012, May 21
	if (only_one==1) {
       strcpy(buffer2,"/tmp/snapshot.jpg");
     }

    /* open file for write */
   // if( (fd = open(buffer2, O_CREAT|O_WRONLY|O_TRUNC, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH)) < 0 ) {
    if( (fd = open("/tmp/snapshot.jpg", O_CREAT|O_WRONLY|O_TRUNC, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH)) < 0 ) {
